					    ...
					    if(url.indexOf(compare.toLowerCase())!=-1 && url.indexOf("shoppingbaba.in")==-1) {
					     ...
					     // message = merchant_data[1] + "," + merchant_data[2];  //shopping baba store + "," + discount
					      panel = '<div id="shoppingbaba_main" style="' + main_css +
									...
									+'<img src = "' + chrome.runtime.getURL("icons/logo.png") + '" style="margin-left:-5.5%; height:95%; width:auto; margin-top:1%; margin-bottom:15%" />'
							...
							//console.log("Ready: " + self.options.ready);
								...
								if(isInIFrame==true){
									...
									elemDiv.style.cssText = 'position:fixed; cursor:pointer;'
															...
															+' margin-top:1px; top:30%; left:-250px; z-index: 99999999;';
									...
									$("#shoppingbaba_tab").on("mouseover", function(){
										...
										if (!animate) {
											animate = true;
											$( '#shoppingbaba_main' ).animate({left: "-2px"}, 500, function(){
												if (animate_return) {
													$( '#shoppingbaba_main' ).animate({left: "-252px"}, 500, function() {animate_return = false;});
													$('#shoppingbaba_tab').animate({left:"-250px"}, 500);	
												}
												animate = false;
											});
											$('#shoppingbaba_tab').animate({left:"4px"}, 500);	
										}
									...
									$("#shoppingbaba_tab").on("mouseout", function(){
										...
										if(!animate) {
											animate = true;
											$( '#shoppingbaba_main' ).animate({left: "-252px"}, 500, function(){
												animate = false;
											});
											$('#shoppingbaba_tab').animate({left:"-250px"}, 500);	
										}
